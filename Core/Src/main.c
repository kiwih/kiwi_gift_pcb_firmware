...
#define LED_BRIGHTNESS_CYCLES 30
	...
	for(uint8_t i = 0; i < LEDS_N; i++) {
		if(leds_status[i].velocity != 0) {
			...
			if((leds_status[i].velocity > 0 && leds_status[i].brightness >= LED_BRIGHTNESS_CYCLES/2) ||
			   (leds_status[i].velocity < 0 && leds_status[i].brightness <= LED_BRIGHTNESS_CYCLES/2)) {
...
uint8_t leds_program_two_fill_step() {
...
void advance_state(enum program_state_e *program_state, uint32_t cycle_count, uint32_t *await_cycle_count) {
	static uint8_t fill = 0;
	static int8_t velocity = 1;
	static int stabletcount = 0;
	if(leds_program_one_step() == 0) {
		if(stabletcount == 0) {
			leds_program_one_eye_init(fill, velocity);
		}
		stabletcount++;
		//leds_status[9].velocity = velocity;
		if(leds_program_one_single_led_step(9) == 0 && stabletcount > 100) {
			if(stabletcount > 10) {
				stabletcount = 0;

				if(velocity == 1) {
					velocity = -1;
					fill = LED_BRIGHTNESS_CYCLES;
				}
				else {
					velocity = 1;
					fill = 0;
...
int main(void)
{
	...
	enum program_state_e program_state = program_one_init;
	...
	while (1)
	{
		...
		if(cycle_count >= await_cycle_count) {
			advance_state(&program_state, cycle_count, &await_cycle_count);
			cycle_count = 0;
		}

		cycle_count++;

		leds_emit((cycle_count % LED_BRIGHTNESS_CYCLES) + 1);
